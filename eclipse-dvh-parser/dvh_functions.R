extract_dvh_data <- function(files) {
  ...
  for(x in files) {
    ...
    n_fields <- max(count_fields(a,tokenizer = tokenizer_delim(delim = " ")))
    ...
    b <- read_table(t,col_names = F,col_types = cols(.default = "c")) 
    ...
    vol_type2 <- str_extract(a,"Ratio of Total Structure Volume \\[\\%\\]")
    vol_type2 <- vol_type2[!is.na(vol_type2)]
    
    # This function will change vol_type object
    vol_type <- ifelse(
      length(vol_type2)>0,str_replace(vol_type,"cm","%"),vol_type
    )
    ...
    bl <- lapply(bl,function(x) x[grepl("^\\d",x$X1),])
    ...
    check_names <- c("relative_volume","absolute_volume","relative_dose","relative_volume")
    ...
    dvhdata <- dvhdata %>% select(id,structure,absolute_dose,absolute_volume,relative_dose,relative_volume,plan,dose_type,vol_type)
    ...
    pointdoses <- pointdoses %>% rename(structure=str_list) %>% select(-plan_name)
    ...
    data %>% 
      mutate(across(c(absolute_dose,absolute_volume,relative_dose,relative_volume),~as.numeric(.))) -> data
    
    id <- str_replace_all(id,"\\/","_") # We cannot use / in the name as it will create a subdirectory. hence we use a _
    ...
    plan_name <- str_to_lower(str_trim(str_replace_all(plan_name," ","_")))
    
    fname <- paste(trimws(id),plan_name,".csv",sep="") # Name for CSV file
